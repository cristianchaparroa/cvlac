...
require_once( 'CVLACScraper.php' );
/**
  *@file
  *
  *Clase encargada de extraer toda la información respecto al
  *CvLAC de un integrante del grupo de Investigación GIIRA.
  * 
  *@author: Cristian Camilo Chaparro A.
  */

class IndividualCVLACScraper extends CVLACScraper 
{
  /**
   * Código del  CvLAC de una paresona en el grupo de investigación GIIRA.
   */
  private $code;
  private $url;
	...
	{
      $this->code = $code;
      $this->url = 'scienti1.colciencias.gov.co:8081/cvlac/visualizador/generarCurriculoCv.do?cod_rh=' . $this->code ;
      CVLACScraper::__construct(  $this->url  );
	}
  ...
  public function extraer($query, $filter = true ){
      $items =  array();
      ...
      foreach( $listaNodos as $element )
      {
          ...
          foreach( $nodeList as $node )
          {
              ...
              $value =  $doc->saveHTML() ;
              $value = str_replace('<blockquote>', '', $value );
              $value = str_replace('</blockquote>>', '', $value );   
